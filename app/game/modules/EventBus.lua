...
function EventBusModule()
    ...
    local function off(id_message, callback)
        local key_message = ensure_hash(id_message)
        if not listeners[key_message] then
            bus_log.warn(("Ни один слушатель для события не зарегистрирован: " .. tostring(id_message)) .. ", off")
            return
        end
    ...
    local function off_all_id_message(id_message)
        local key_message = ensure_hash(id_message)
        if not listeners[key_message] then
            bus_log.warn(("Ни один слушатель для события не зарегистрирован: " .. tostring(id_message)) .. ", off_all_id_message")
            return
        end
    ...
    local function trigger(id_message, message_data, show_warning, is_copy_data)
        ...
        if not listeners[key_message] then
            if show_warning then
                bus_log.warn(("Ни один слушатель для события не зарегистрирован: " .. tostring(id_message)) .. ", trigger/send")
            end
            return
        end
