    ...
    timer.delay(is_animal_level() ? GAME_CONFIG.animal_level_delay_before_win : GAME_CONFIG.delay_before_win, false, () => {
        ...
        gui.animate(lock, gui.PROP_COLOR, vmath.vector4(0, 0, 0, GAME_CONFIG.fade_value), gui.EASING_INCUBIC, 0.6, 0, () => {
            ...
            if (data.with_reward) {
                ...
                let steps = (data.state.steps != undefined) ? math.min(data.state.steps, GAME_CONFIG.max_coins_for_reward) : 0;
                let remaining_time = (data.state.remaining_time != undefined) ? math.min(math.floor(data.state.remaining_time), GAME_CONFIG.max_coins_for_reward) : 0;
                if (level_coins > 0) {
